...
async function legacyRecalculateAllElo(): Promise<void> {
  ...
  for (const match of confirmedMatches) {
    ...
    if (clubId == null) continue;
    if (!eloByClub[clubId]) eloByClub[clubId] = {};
    const playersInMatch = await db.select().from(matchPlayersTable).where(eq(matchPlayersTable.matchId, match.id));
  ...
  for (const player of players) {
    ...
    await db.update(playersTable).set({ elo: finalElo }).where(eq(playersTable.id, player.id));
  }
}
...
async function enrichMatch(m: typeof matchesTable.$inferSelect) {
  ...
  const eloChanges = filteredHistory.map((historyItem) => ({ playerId: historyItem.playerId, playerName: playerMap[historyItem.playerId] ?? "Desconocido", eloBefore: historyItem.eloBefore, eloAfter: historyItem.eloAfter, eloChange: historyItem.eloChange }));
  const team1Players = filteredMatchPlayers.filter((player) => player.team === "team1").map((player) => ({ id: player.playerId, name: playerMap[player.playerId] ?? "Desconocido" }));
  const team2Players = filteredMatchPlayers.filter((player) => player.team === "team2").map((player) => ({ id: player.playerId, name: playerMap[player.playerId] ?? "Desconocido" }));
...
router.post("/matches", requireCommunityAccess, async (req, res): Promise<void> => {
  ...
  if (!modality) { res.status(400).json({ error: "La modalidad no pertenece al deporte seleccionado" }); return; }
  ...
  if (new Set(allIds).size !== allIds.length) { res.status(400).json({ error: "Un jugador no puede estar en ambos equipos" }); return; }
  ...
  if (!Number.isInteger(clubId) || clubId! <= 0) { res.status(400).json({ error: "Los Super Admin deben indicar clubId para crear un partido" }); return; }
  if (encuentroId !== undefined && encuentroId !== null) {
    const parsedEncuentroId = Number(encuentroId);
    if (isNaN(parsedEncuentroId) || !Number.isInteger(parsedEncuentroId) || parsedEncuentroId <= 0) { res.status(400).json({ error: "encuentroId debe ser un número válido" }); return; }
    const [encuentro] = await db.select().from(encuentrosTable).where(eq(encuentrosTable.id, parsedEncuentroId));
    if (!encuentro) { res.status(404).json({ error: "Encuentro no encontrado" }); return; }
    if (encuentro.clubId !== clubId) { res.status(403).json({ error: "El encuentro no pertenece al club autorizado" }); return; }
  }
  const clubPlayers = await db.select({ id: playersTable.id }).from(playersTable).where(eq(playersTable.clubId, clubId));
  ...
  if (invalidPlayers.length > 0) { res.status(400).json({ error: "Uno o más jugadores no pertenecen al club actual", playerIds: invalidPlayers }); return; }
  if (!(await validatePlayersForClub(allIds, clubId))) { res.status(400).json({ error: "Uno o más jugadores no pertenecen al club actual" }); return; }
  const t1Score = team1Score ?? 0;
  const t2Score = team2Score ?? 0;
  let result = "draw";
  if (t1Score > t2Score) result = "team1"; else if (t2Score > t1Score) result = "team2";
...
router.post("/matches/:id/confirm", requireCommunityAccess, async (req, res): Promise<void> => {
  ...
  if (!isSuperAdminUser(req.user) && existing.clubId !== clubId) { res.status(403).json({ error: "Forbidden" }); return; }
  if ((existing as any).status === "confirmed") { res.json(await enrichMatch(existing)); return; }
  const matchPlayers = await db.select().from(matchPlayersTable).where(eq(matchPlayersTable.matchId, id));
  const team1 = matchPlayers.filter((player) => player.team === "team1");
  const team2 = matchPlayers.filter((player) => player.team === "team2");
  if (team1.length === 0 || team2.length === 0) { res.status(400).json({ error: "No se puede confirmar un partido sin jugadores en ambos equipos" }); return; }
  ...
  const [updated] = await db.update(matchesTable).set({ status: "confirmed" } as any).where(eq(matchesTable.id, id)).returning();
  await recalculateAllElo();
  res.json(await enrichMatch(updated));
});

router.get("/matches/:id", requireCommunityAccess, async (req, res): Promise<void> => {
  const rawId = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
  const id = parseInt(rawId, 10);
  if (isNaN(id)) { res.status(400).json({ error: "ID inválido" }); return; }
  const clubId = (req.user as { clubId: number }).clubId;
  ...
  if (!isSuperAdminUser(req.user) && match.clubId !== clubId) { res.status(403).json({ error: "Forbidden" }); return; }
...
router.patch("/matches/:id", requireCommunityAccess, async (req, res): Promise<void> => {
  ...
  if (team1PlayerIds !== undefined || team2PlayerIds !== undefined) {
    ...
    if (new Set(allIds).size !== allIds.length) { res.status(400).json({ error: "Un jugador no puede estar en ambos equipos" }); return; }
    if (newTeam1Ids.length === 0 || newTeam2Ids.length === 0) { res.status(400).json({ error: "Cada equipo debe tener al menos un jugador" }); return; }
    if (newTeam1Ids.length !== modality.teamSize || newTeam2Ids.length !== modality.teamSize) { res.status(400).json({ error: `Cada equipo debe tener exactamente ${modality.teamSize} jugador(es)` }); return; }
    const clubPlayers = await db.select({ id: playersTable.id }).from(playersTable).where(eq(playersTable.clubId, clubId));
    const clubPlayerIds = new Set(clubPlayers.map((player) => player.id));
    const invalidPlayers = allIds.filter((playerId: number) => !clubPlayerIds.has(playerId));
    if (invalidPlayers.length > 0) { res.status(400).json({ error: "Uno o más jugadores no pertenecen al club actual", playerIds: invalidPlayers }); return; }
  ...
  updates.result = resultingTeam1Score > resultingTeam2Score ? "team1" : resultingTeam2Score > resultingTeam1Score ? "team2" : "draw";
  ...
  if (newTeam1Ids !== null && newTeam2Ids !== null) {
    await db.insert(playerSportRatingsTable).values([...newTeam1Ids, ...newTeam2Ids].map((playerId) => ({ playerId, sportId: existing.sportId, elo: STARTING_ELO }))).onConflictDoNothing({ target: [playerSportRatingsTable.playerId, playerSportRatingsTable.sportId] });
    await db.delete(matchPlayersTable).where(eq(matchPlayersTable.matchId, id));
    await db.insert(matchPlayersTable).values([...newTeam1Ids.map((playerId: number) => ({ matchId: id, playerId, team: "team1" })), ...newTeam2Ids.map((playerId: number) => ({ matchId: id, playerId, team: "team2" }))]);
  }
  await recalculateAllElo();
  res.json(await enrichMatch(updated));
});

router.delete("/matches/:id", requireCommunityAccess, async (req, res): Promise<void> => {
  const rawId = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
  const id = parseInt(rawId, 10);
  if (isNaN(id)) { res.status(400).json({ error: "ID inválido" }); return; }
  const clubId = (req.user as { clubId: number }).clubId;
